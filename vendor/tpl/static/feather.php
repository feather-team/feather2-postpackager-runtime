...
if($type){
    if(!$this->get('FEATHER_HEAD_RESOURCE_LOADED')){
        foreach($this->get('FEATHER_USE_STYLES') as $key => $value){
            $output[] = '<link rel="stylesheet" href="' . $value . '" type="text/css" />';
        }

        foreach($this->get('FEATHER_USE_HEAD_SCRIPTS') as $key => $value){
            $output[] = '<script src="' . $value . '"></script>';
        }

        foreach($this->get('FEATHER_USE_HEAD_INLINE_SCRIPTS') as $key => $value){
            $output[] = $value;
        }
    ...
    if(!$this->get('FEATHER_BOTTOM_RESOURCE_LOADED')){
        foreach($this->get('FEATHER_USE_SCRIPTS') as $key => $value){
            $output[] = '<script src="' . $value . '"></script>';
        }
